# CLAUDE.md — Activeledger Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Activeledger Ai
- **Role**: AI-powered Activeledger companion
- **URL**: https://activeledger-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/activeledger-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/activeledger-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://activeledger-ai.casey-digennaro.workers.dev/health
curl -s https://activeledger-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=activeledger-ai
...
- **vessel.json capabilities**: finance-tracking, transaction-management
...
1. Keep the vessel small. ~248 lines is the current size.
